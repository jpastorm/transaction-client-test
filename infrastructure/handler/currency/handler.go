...
func newHandler(useCase currency.UseCase, response response.Responser) handler {
	return handler{useCase: useCase, response: response}
}

// Create handles the creation of a model.Currency
...
// Update handles the update of a model.Currency
...
// Delete handles the deleting of a model.Currency
...
// GetWhere handles the search of a model.Currency
func (h handler) GetWhere(c echo.Context) error {
	...
	currencyData, err := h.useCase.GetAllWhere(filtersSpecification)
	if err != nil {
		return h.response.Error(c, "useCase.GetWhere()", err)
	}

	return c.JSON(h.response.OK(currencyData))
}

// GetAllWhere handles the search of all model.Currency
func (h handler) GetAllWhere(c echo.Context) error {
	...
	currency, err := h.useCase.GetAllWhere(filtersSpecification)
	if err != nil {
		return h.response.Error(c, "useCase.GetAllWhere()", err)
	}

	return c.JSON(h.response.OK(currency))
}
